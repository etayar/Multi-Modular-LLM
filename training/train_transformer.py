...
class TransformerTrainer:
    ...
    def __init__(self, config: dict):
        ...
        self.config["__run_time__"] = config.get("resume_from_date") or datetime.now().strftime(
            "%Y-%m-%d_%H-%M-%S"
        )
            ...
            class WikipediaStream(IterableDataset):
                def __init__(self, tokenizer, chunk_len, max_articles=None):
                    ...
                    self.dataset = load_dataset(
                        "wikipedia",
                        cfg.get("dataset_config", "20220301.en"),
                        split="train",
                        streaming=True,
                    )
                ...
                def __iter__(self):
                    ...
                    for ex in self.dataset:
                        ...
                        tokens = self.tokenizer(
                            ex["text"],
                            truncation=True,
                            max_length=min(self.chunk_len * 10, 2048),
    ...
    def evaluate(self, num_batches=None):
        ...
        for i, batch in tqdm(
            enumerate(self.dataloader), total=num_batches, desc="Evaluating", leave=False
        ):
            if i >= num_batches:
                break
            try:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config["epochs"] + 1):
            # recreate loader each epoch (fresh iterator)
            self.dataloader = self._build_dataloader()
            ...
            print(
                f"Epoch {epoch} Complete — Train {avg_train:.4f} | Eval {eval_loss:.4f} | PPL {ppl:.2f}"
            )
        ...
        # --------------- save loss history ----------- #
        (self.log_dir / "loss_history.json").write_text(
            json.dumps(
                {
